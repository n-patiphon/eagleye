...
def rotate_vector(vector, roll, pitch, yaw):
    """Rotate a 3D vector using roll, pitch, and yaw angles (in radians)."""
    ...
    rotated_vector = R_z @ R_y @ R_x @ vector
    return rotated_vector


def calc_dr_eagleye_3d(TimeStamp,distance,eagleye_twist_data,ref_rpy,ref_xyz,distance_length,distance_step,based_heaing_angle):

    ref_heading = np.deg2rad(ref_rpy["yaw"])
    ref_rolling = np.deg2rad(ref_rpy["roll"])
    ref_pitching = np.deg2rad(ref_rpy["pitch"])

    last_distance = 0
    last_heading_integtation = 0
    last_roll_integtation = 0
    last_pitch_integtation = 0
    velocity_stop_threshold = 0.01
    velocity_slip_threshold = 1
    set_dr_trajcetory: List[float] = []
    set_calc_error: List[float] = []
    for i in range(len(TimeStamp)):
        ...
        if last_distance + distance_step < distance[i]:
            ...
            for j in range(i,len(TimeStamp)):
                ...
                if data_set_flag == True:
                    last_time = TimeStamp[j]
                    last_heading_integtation = ref_heading[j]
                    last_roll_integtation = ref_rolling[j]
                    last_pitch_integtation = ref_pitching[j]
                    distance_data = distance[j] - start_distance
                    absolute_pos_x = ref_xyz['x'][j] - start_pos_x
                    absolute_pos_y = ref_xyz['y'][j] - start_pos_y
                    absolute_pos_z = ref_xyz['z'][j] - start_pos_z
                    data_set_flag = False
                    continue
                if eagleye_twist_data['velocity'][j] > velocity_stop_threshold:
                    heading_integtation = last_heading_integtation + (eagleye_twist_data['angular_z'][j] + eagleye_twist_data['yawrate_offset'][j]) * (TimeStamp[j] - last_time)
                ...
                if eagleye_twist_data['velocity'][j] > velocity_slip_threshold:
                    ...
                    heading_correction_slip = heading_integtation
                rolling_integtation = last_roll_integtation + eagleye_twist_data['angular_x'][j] * (TimeStamp[j] - last_time)
                pitching_integtation = last_pitch_integtation + eagleye_twist_data['angular_y'][j] * (TimeStamp[j] - last_time)

                if distance[j] - start_distance < distance_length:
                    # if based_heaing_angle == True:
                    #     #usr_vel_x = math.sin(heading_correction_slip) * eagleye_twist_data['velocity'][j]
                    #     #usr_vel_y = math.cos(heading_correction_slip) * eagleye_twist_data['velocity'][j]
                    #     velocity = eagleye_twist_data['velocity'][j]
                    #     velocity_vector = np.array([math.sin(heading_correction_slip) * velocity,
                    #                                 math.cos(heading_correction_slip) * velocity,
                    #                                 0])
                    #     rotated_velocity = rotate_vector(velocity_vector, rolling_integtation, pitching_integtation, heading_correction_slip)
                    #     usr_vel_x = rotated_velocity[0]
                    #     usr_vel_y = rotated_velocity[1]
                    #     usr_vel_z = rotated_velocity[2]
                    # else:
                    #     #usr_vel_x = math.cos(heading_correction_slip) * eagleye_twist_data['velocity'][j]
                    #     #usr_vel_y = math.sin(heading_correction_slip) * eagleye_twist_data['velocity'][j]
                    #     velocity = eagleye_twist_data['velocity'][j]
                    #     velocity_vector = np.array([math.cos(heading_correction_slip) * velocity,
                    #                                 math.sin(heading_correction_slip) * velocity,
                    #                                 0])
                    #     rotated_velocity = rotate_vector(velocity_vector, rolling_integtation, pitching_integtation, heading_correction_slip)
                    #     usr_vel_x = rotated_velocity[0]
                    #     usr_vel_y = rotated_velocity[1]
                    #     usr_vel_z = rotated_velocity[2]
                    # 前の位置 (x, y, z)
                    previous_position = np.array([previous_pos_x, previous_pos_y, previous_pos_z])

                    # 車速 (m/s)
                    velocity = eagleye_twist_data['velocity'][j]

                    yaw = math.fmod((90 * math.pi / 180) - heading_correction_slip, 2 * math.pi)

                    # roll, pitch, yaw (rad)
                    roll, pitch, yaw = rolling_integtation, pitching_integtation, yaw

                    # 時間経過 (s)
                    dt = (TimeStamp[j] - last_time)
                    ...
                    # 速度ベクトルを更新
                    delta_position = rotation_matrix.dot(np.array([velocity * dt, 0, 0]))
                    ...
                    # dr_pos_x = previous_pos_x + usr_vel_x * (TimeStamp[j] - last_time)
                    # dr_pos_y = previous_pos_y + usr_vel_y * (TimeStamp[j] - last_time)
                    # dr_pos_z = previous_pos_z + usr_vel_z * (TimeStamp[j] - last_time)
                    dr_pos_x = current_position[0]
                    dr_pos_y = current_position[1]
                    dr_pos_z = current_position[2]

                    last_heading_integtation = heading_correction_slip
                    previous_pos_x = dr_pos_x
                    previous_pos_y = dr_pos_y
                    previous_pos_z = dr_pos_z
                    last_time = TimeStamp[j]
                    distance_data = distance[j] - start_distance
                    absolute_pos_x = ref_xyz['x'][j] - start_pos_x
                    absolute_pos_y = ref_xyz['y'][j] - start_pos_y
                    ...
                    absolute_dr_pos_x = start_pos_x + dr_pos_x
                    absolute_dr_pos_y = start_pos_y + dr_pos_y
                    absolute_dr_pos_z = start_pos_z + dr_pos_z
                    set_dr_trajcetory.append([absolute_dr_pos_x,absolute_dr_pos_y, absolute_dr_pos_z])
                else:
                    error_x = absolute_pos_x - dr_pos_x
                    error_y = absolute_pos_y - dr_pos_y
                    error_2d_fabs = math.fabs(error_x ** 2 + error_y ** 2)
        ...
        # break
    calc_error = pd.DataFrame(set_calc_error,columns=['start_distance','distance','absolute_pos_x','absolute_pos_y','dr_pos_x','dr_pos_y','error_x','error_y','error_2d'])
    dr_trajcetory = pd.DataFrame(set_dr_trajcetory,columns=['x','y','z'])
    return calc_error,dr_trajcetory
